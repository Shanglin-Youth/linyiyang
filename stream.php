...
if($postData_arr["model"]=="glm-4-flash"){
	...
	$OPENAI_API_KEY = "Bearer "."ccCVqXgjyCyimRLHpWwT:WGRpRXWIewdXIWqjgaep";//API密钥，APIPassword
	}else if($postData_arr["model"]=="xinghuo"){
	//讯飞星火（Spark Max）收费
	$my_CURLOPT_URL="https://spark-api-open.xf-yun.com/v1/chat/completions";//请求地址
...
function check_fromurl(){   
  ...
  //如果页面的域名不是服务器域名,就拒绝
  if($_SERVER['HTTP_HOST']   !=   $urlar["host"]   &&   $urlar["host"]   !=   "ai.shanglin.cloud"   &&   $urlar["host"]   !=   "http:$url")   {
  //header("location:/");
  echo "failed";
  exit;   
  }     
